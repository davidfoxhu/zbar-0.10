...
int _zbar_processor_open (zbar_processor_t *proc, char *title, unsigned width, unsigned height)
{
	...
	if (!GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, (void*)_zbar_processor_open, (HINSTANCE*)&hmod))
	{
...
int _zbar_processor_set_size (zbar_processor_t *proc, unsigned width, unsigned height)
{
    ...
    AdjustWindowRectEx(&r, GetWindowLong(proc->display, GWL_STYLE), 0, GetWindowLong(proc->display, GWL_EXSTYLE));
	if (!SetWindowPos(proc->display, NULL, 0, 0, r.right - r.left, r.bottom - r.top, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOZORDER | SWP_NOREPOSITION))
	{
		return(-1/*FIXME*/);
	}
