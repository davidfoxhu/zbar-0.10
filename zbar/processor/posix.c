...
int _zbar_thread_start (zbar_thread_t *thr, zbar_thread_proc_t *proc, void *arg, zbar_mutex_t *lock)
{
	...
	if(pthread_create(&thr->tid, NULL, proc, arg) || _zbar_event_wait(&thr->activity, lock, NULL) < 0 || !thr->running)
	{
		thr->started = 0;
		_zbar_event_destroy(&thr->notify);
		_zbar_event_destroy(&thr->activity);
		/*rc = err_capture_num(proc, SEV_ERROR, ZBAR_ERR_SYSTEM, __func__, "spawning thread", rc);*/
		rc = -1/*FIXME*/;
	}
...
static inline void proc_cache_polling (processor_state_t *state)
{
	...
	memcpy(state->thr_polling.fds, state->polling.fds, n * sizeof(struct pollfd));
	memcpy(state->thr_polling.handlers, state->polling.handlers, n * sizeof(poll_handler_t*));
}
...
int _zbar_processor_init (zbar_processor_t *proc)
{
	...
	if(proc->threaded)
	{
		...
		{
			return(err_capture(proc, SEV_FATAL, ZBAR_ERR_SYSTEM, __func__, "failed to open pipe"));
		}
