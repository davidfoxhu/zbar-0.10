...
static inline void proc_waiter_release (zbar_processor_t *proc, proc_waiter_t *waiter)
{
    if(waiter)
	{
...
void _zbar_processor_notify (zbar_processor_t *proc, unsigned events)
{
	...
	{
		waiter->events = ((waiter->events & ~events) | (events & EVENT_CANCELED));
	}
    if(!proc->lock_level)
	{
...
static inline int proc_wait_unthreaded (zbar_processor_t *proc, proc_waiter_t *waiter, zbar_timer_t *timeout)
{
