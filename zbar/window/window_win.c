...
int _zbar_window_resize (zbar_window_t *w)
{
	...
	win->logo_zpen = ExtCreatePen(PS_GEOMETRIC | PS_SOLID | PS_ENDCAP_ROUND | PS_JOIN_ROUND, lbw * 2, &lb, 0, NULL);

	lb.lbColor = RGB(0xa4, 0x00, 0x00);
	win->logo_zbpen = ExtCreatePen(PS_GEOMETRIC | PS_SOLID | PS_ENDCAP_ROUND | PS_JOIN_ROUND, lbw * 2, &lb, 0, NULL);
...
int _zbar_window_attach (zbar_window_t *w, void *display, unsigned long unused)
{
	...
	win->bih.biXPelsPerMeter = 1000L * GetDeviceCaps(hdc, HORZRES) / GetDeviceCaps(hdc, HORZSIZE);
	win->bih.biYPelsPerMeter = 1000L * GetDeviceCaps(hdc, VERTRES) / GetDeviceCaps(hdc, VERTSIZE);

	int height = -MulDiv(11, GetDeviceCaps(hdc, LOGPIXELSY), 96);
	HFONT font = CreateFontW(height, 0, 0, 0, 0, 0, 0, 0, ANSI_CHARSET, 0, 0, 0, FF_MODERN | FIXED_PITCH, NULL);
...
static inline void win_set_rgb (HDC hdc, uint32_t rgb)
{
	...
	SetDCPenColor(hdc, RGB((rgb & 4) * 0x33, (rgb & 2) * 0x66, (rgb & 1) * 0xcc));
}
...
int _zbar_window_draw_polygon (zbar_window_t *w, uint32_t rgb, const point_t *pts, int npts)
{
	...
	/***ºúÑÇÃÏ***/
	/*POINT	gdipts[npts + 1];*/
	POINT*	gdipts = (POINT*)malloc(sizeof(POINT)*(npts+1));
	int i;
...
int _zbar_window_draw_text (zbar_window_t *w, uint32_t rgb, point_t p, const char *text)
{
	...
	SetTextColor(hdc, RGB((rgb & 4) * 0x33, (rgb & 2) * 0x66, (rgb & 1) * 0xcc));
...
int _zbar_window_fill_rect (zbar_window_t *w, uint32_t rgb, point_t org, point_t size)
{
	...
	SetDCBrushColor(hdc, RGB((rgb & 4) * 0x33, (rgb & 2) * 0x66, (rgb & 1) * 0xcc));
