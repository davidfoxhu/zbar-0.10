...
static inline unsigned get_width (const zbar_decoder_t *dcode, unsigned char offset)
{
    return(dcode->w[(dcode->idx - offset) & (DECODE_WINDOW - 1)]);
}

/* retrieve bar+space pair width starting at offset i */
static inline unsigned pair_width (const zbar_decoder_t *dcode, unsigned char offset)
{
 ...
 */
static inline unsigned calc_s (const zbar_decoder_t *dcode, unsigned char offset, unsigned char n)
{
...
static inline char get_lock (zbar_decoder_t *dcode, zbar_symbol_type_t req)
{
	if (dcode->lock)
	{
		return(1);
 ...
 */
static __inline unsigned decode_sort3(zbar_decoder_t *dcode, int i0)
{
 ...
 */
static __inline unsigned decode_sortn(zbar_decoder_t *dcode, int n, int i0)
{
...
/* acquire shared state lock */
static __inline char acquire_lock(zbar_decoder_t *dcode, zbar_symbol_type_t req)
{
	if (dcode->lock)
	{
		return(1);
...
static __inline char release_lock(zbar_decoder_t *dcode, zbar_symbol_type_t req)
{
	dcode->lock = 0;
...
extern const char *_zbar_decoder_buf_dump (unsigned char *buf, unsigned int buflen);
