...
static __inline int check_width (unsigned cur, unsigned prev)
{
    unsigned dw;
...
static __inline int encode6 (zbar_decoder_t *dcode)
{
...
static __inline int validate_sig (int sig)
{
...
static __inline int decode6 (zbar_decoder_t *dcode)
{
    int sig = encode6(dcode);
    int g0, g1, c;

	if (sig < 0 || (sig & 0x3) + ((sig >> 4) & 0x3) + ((sig >> 8) & 0x3) != 3 || validate_sig(sig))
	{
		return(-1);
	}
    ...
    zassert(g0 >= 0 && g1 >= 0, -1, "dir=%x sig=%03x g0=%03x g1=%03x %s\n", dcode->code93.direction, sig, g0, g1, _zbar_decoder_buf_dump(dcode->buf, dcode->code93.character));
...
static __inline zbar_symbol_type_t decode_start (zbar_decoder_t *dcode)
{
...
static __inline zbar_symbol_type_t decode_abort (zbar_decoder_t *dcode, const char *reason)
{
    code93_decoder_t *dcode93 = &dcode->code93;
...
static __inline zbar_symbol_type_t check_stop (zbar_decoder_t *dcode)
{
    code93_decoder_t *dcode93 = &dcode->code93;
    unsigned n = dcode93->character, s = dcode->s6;
    int max_len = CFG(*dcode93, ZBAR_CFG_MAX_LEN);

	if (n < 2 || n < CFG(*dcode93, ZBAR_CFG_MIN_LEN) || (max_len && n > max_len))
	{
...
static __inline int plusmod47 (int acc, int add)
{
    acc += add;
	if (acc >= CHKMOD)
	{
...
static __inline int validate_checksums (zbar_decoder_t *dcode)
{
    code93_decoder_t *dcode93 = &dcode->code93;
...
static __inline int postprocess (zbar_decoder_t *dcode)
{
    code93_decoder_t *dcode93 = &dcode->code93;
    unsigned i, j, n = dcode93->character;
    static const unsigned char code93_graph[] = "-. $/+%";
    static const unsigned char code93_s2[] = "\x1b\x1c\x1d\x1e\x1f;<=>?[\\]^_{|}~\x7f\x00\x40`\x7f\x7f\x7f";
    ...
    zassert(j < dcode->buf_alloc, 1, "j=%02x %s\n", j, _zbar_decoder_buf_dump(dcode->buf, dcode->code93.character));
    dcode->buflen = j;
    dcode->buf[j] = '\0';
    dcode->modifiers = 0;
...
zbar_symbol_type_t _zbar_decode_code93 (zbar_decoder_t *dcode)
{
	...
	if ( ++dcode93->element != 6 /* process every 6th element of active symbol */ ||
		get_color(dcode) == dcode93->direction /* decode color based on direction */ )
	{
		return(ZBAR_NONE);
	}
    dcode93->element = 0;
