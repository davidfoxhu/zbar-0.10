...
static __inline void append_check14 (unsigned char *buf)
{
...
static __inline void decode10 (unsigned char *buf, unsigned long n, int i)
{
    buf += i;
    while(--i >= 0)
	{
...
static __inline int databar_postprocess_exp (zbar_decoder_t *dcode, int *data)
{
	ptrdiff_t i = 0;
	int enc = 0;
    unsigned n;
 ...
 */
static __inline void databar_postprocess (zbar_decoder_t *dcode, unsigned d[4])
{
...
static __inline int check_width (unsigned wf, unsigned wd, unsigned n)
{
    unsigned dwf = wf * 3;
...
static __inline void merge_segment (databar_decoder_t *db, databar_segment_t *seg)
{
...
static __inline zbar_symbol_type_t match_segment (zbar_decoder_t *dcode, databar_segment_t *seg)
{
    databar_decoder_t *db = &dcode->databar;
...
static __inline unsigned lookup_sequence (databar_segment_t *seg, int fixed, int seq[22])
{
...
static __inline zbar_symbol_type_t match_segment_exp (zbar_decoder_t *dcode, databar_segment_t *seg, int dir)
{
    databar_decoder_t *db = &dcode->databar;
    ...
    for(i = 0; ; i--)
	{
        ...
        for(; i >= 0 && seq[i] >= 0; i--)
		{
            ...
            else
			{
				...
				{
                    if(iseg[j] == seq[i] && (!i || check_width(width, db->segs[j].width, 14)))
					{
                        seg = db->segs + j;
                        break;
                    }
...
static __inline unsigned calc_check (unsigned sig0, unsigned sig1, unsigned side, unsigned mod)
{
    unsigned chk = 0;
    int i;
...
static __inline int calc_value4 (unsigned sig, unsigned n, unsigned wmax, unsigned nonarrow)
{
    unsigned v = 0;
...
static __inline zbar_symbol_type_t decode_char (zbar_decoder_t *dcode, databar_segment_t *seg, int off, int dir)
{
    databar_decoder_t *db = &dcode->databar;
...
static __inline int alloc_segment (databar_decoder_t *db)
{
...
static __inline zbar_symbol_type_t decode_finder (zbar_decoder_t *dcode)
{
    databar_decoder_t *db = &dcode->databar;
    databar_segment_t *seg;
