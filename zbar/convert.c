...
static inline int verify_format_sort (void)
{
    ...
    for(i = 0; i < num_format_defs; i++)
	{
		...
		if ((j < num_format_defs && format_defs[i].format < format_defs[j].format) || (j + 1 < num_format_defs && format_defs[j + 1].format < format_defs[i].format))
		{
			break;
		}
    }
...
static inline unsigned long uvp_size (const zbar_image_t *img, const zbar_format_def_t *fmt)
{
	if (fmt->group == ZBAR_FMT_GRAY)
	{
		return(0);
    ...
    return((img->width >> fmt->p.yuv.xsub2) * (img->height >> fmt->p.yuv.ysub2));
}
 ...
 */
static inline void convert_y_resize (zbar_image_t *dst, const zbar_format_def_t *dstfmt, const zbar_image_t *src, const zbar_format_def_t *srcfmt, size_t n)
{
...
static void convert_uvp_append (zbar_image_t *dst, const zbar_format_def_t *dstfmt, const zbar_image_t *src, const zbar_format_def_t *srcfmt)
{
    ...
    zprintf(24, "dst=%dx%d (%lx) %lx src=%dx%d %lx\n", dst->width, dst->height, n, dst->datalen, src->width, src->height, src->datalen);
...
zbar_image_t *zbar_image_convert_resize (const zbar_image_t *src, unsigned long fmt, unsigned width, unsigned height)
{
    ...
    conversion_handler_t *func = conversions[srcfmt->group][dstfmt->group].func;
...
int _zbar_best_format (uint32_t src, uint32_t *dst, const uint32_t *dsts)
{
    ...
    for(; *dsts; dsts++)
	{
		...
		{
			printf(" %.4s(%08i )=%d", (char*)dsts, *dsts, cost);
		}
...
{
	return(err_capture(0, SEV_ERROR, ZBAR_ERR_UNSUPPORTED, __func__, "no supported image formats available"));
/*	if (!vdo && !win)
	{
		return(0);
	}
    ...
    for(fmt = _zbar_formats; *fmt; fmt++)
	{
        ...
        if(cost < 0)
		{
            zprintf(4, "%.4s(%08 ) -> ? (unsupported)\n", (char*)fmt, *fmt);
            continue;
        }
        zprintf(4, "%.4s(%08 ) -> %.4s(%08 ) (%d)\n", (char*)fmt, *fmt, (char*)&win_fmt, win_fmt, cost);
        if(min_cost > cost)
		{
            min_cost = cost;
            min_fmt = *fmt;
    ...
    zprintf(2, "setting best format %.4s(%08) (%d)\n", (char*)&min_fmt, min_fmt, min_cost);
