...
int zbar_image_write(const zbar_image_t *img, const char *filebase)
{
	...
	{
		n = snprintf(filename, len, "%s.%08i.zimg", filebase, img->format);
	}
    assert(n < len);
    filename[len] = '\0';
    ...
    if(fwrite(&hdr, sizeof(hdr), 1, f) != 1 || fwrite(img->data, 1, img->datalen, f) != img->datalen)
	{
#ifdef HAVE_ERRNO_H
		rc = errno;
...
int zbar_image_write_png (const zbar_image_t *img, const char *filename)
{
    ...
    png_set_IHDR(png, info, img->width, img->height, 8, PNG_COLOR_TYPE_GRAY, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
