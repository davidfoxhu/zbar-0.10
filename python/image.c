...
static int image_set_symbols (zbarImage *self, PyObject *value, void *closure)
{
	...
	{
		PyErr_Format(PyExc_TypeError, "must set image symbols to a zbar.SymbolSet, not '%.50s'", value->ob_type->tp_name);
		return(-1);
	}
...
static int image_set_format (zbarImage *self, PyObject *value, void *closure)
{
	...
	if(PyBytes_AsStringAndSize(value, &format, &len) || !format || len != 4)
	{
		PyErr_Format(PyExc_ValueError, "format '%.50s' is not a valid four character code", format);
		return(-1);
	}
...
static int image_set_size (zbarImage *self, PyObject *value, void *closure)
{
	...
	int width = PyLong_AsSsize_t(wobj);
	...
	int height = PyLong_AsSsize_t(hobj);
...
error:
	...
	if (rc)
	{
		PyErr_SetString(PyExc_ValueError, "size must be a sequence of two ints");
	}
...
static int image_init (zbarImage *self, PyObject *args, PyObject *kwds)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwds, "|iiOO", kwlist, &width, &height, &format, &data))
	{
		return(-1);
	}
	if (width > 0 && height > 0)
...
static zbarImage* image_convert (zbarImage *self, PyObject *args, PyObject *kwds)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|ii", kwlist, &format, &width, &height))
	{
		return(NULL);
	}
	assert(format);
	...
	if (width > 0 && height > 0)
	{
		img->zimg = zbar_image_convert_resize(self->zimg, *((unsigned long*)format), width, height);
	}
	else
	{
