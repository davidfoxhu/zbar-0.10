...
static char scanner_doc[] = PyDoc_STR("low level intensity sample stream scanner.  identifies \"bar\" edges and measures width between them.\n\nFIXME.");

static zbarScanner* scanner_new (PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	zbarDecoder *decoder = NULL;
	static char *kwlist[] = { "decoder", NULL };

	if (!PyArg_ParseTupleAndKeywords(args, kwds, "|O!", kwlist, &decoder, zbarDecoder_Type))
	{
		return(NULL);
	}
...
static zbarEnumItem* scanner_scan_y (zbarScanner *self, PyObject *args, PyObject *kwds)
{
...
{
    { "reset",        (PyCFunction)scanner_reset,		METH_VARARGS | METH_KEYWORDS, },
    { "new_scan",     (PyCFunction)scanner_new_scan,	METH_VARARGS | METH_KEYWORDS, },
    { "scan_y",       (PyCFunction)scanner_scan_y,		METH_VARARGS | METH_KEYWORDS, },
    { NULL, },
};
