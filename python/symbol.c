...
static PyObject* symbol_get_data (zbarSymbol *self, void *closure)
{
	if(!self->data)
	{
		...
		/********************************ºúÑÇÃÏ********************/
		/*self->data = PyString_FromStringAndSize(zbar_symbol_get_data(self->zsym), zbar_symbol_get_data_length(self->zsym));*/
		self->data = PyBytes_FromStringAndSize(zbar_symbol_get_data(self->zsym), zbar_symbol_get_data_length(self->zsym));
		if (!self->data)
		{
			return(NULL);
		}
