...
static inline PyObject* exc_get_message (zbarException *self, void *closure)
{
	...
	/*if(!PyString_Size(super->cause))*/
	if(!PyBytes_Size(super->cause))
	{
		Py_CLEAR(super->cause);
		if (!self->obj || !zbarProcessor_Check(self->obj))
		{
			/*super->cause = PyString_FromString("unknown zbar error");*/
			super->cause = PyBytes_FromString("unknown zbar error");
		}
		else
		{
			const void *zobj = ((zbarProcessor*)self->obj)->zproc;

			/*(super->cause = PyString_FromString(_zbar_error_string(zobj, 1));*/
			super->cause = PyBytes_FromString("unknown zbar error");
		}
	}
	Py_INCREF(super->cause);
...
static int exc_init (zbarException *self, PyObject *args, PyObject *kwds)
{
	...
	if (!_PyArg_NoKeywords(self->base.ob_base.ob_type->tp_name, kwds))
	{
		return(-1);
	}
